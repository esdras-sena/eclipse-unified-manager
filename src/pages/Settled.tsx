...
import { useSettledQueries } from "@/web3/hooks";
...
const Settled = () => {
  ...
  const { queries: rawQueries, loading, error } = useSettledQueries();

  const filteredQueries = useMemo(() => {
    let filtered: SettledQuery[] = rawQueries.map(q => ({
      id: q.id,
      title: q.title,
      ...
      settledTime: q.settledTime,
      settledTimeUnix: q.settledTimeUnix,
